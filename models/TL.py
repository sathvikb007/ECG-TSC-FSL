...
#TODO: Add comments to this file, training, fix testing, etc.
...
class Transfer_Learning_Model:
    ...
    def __init__(self, retrain = False, output_dir = '../weights/', batch_size = None, epochs = None, verbose = None):
        """
            ...
            batch_size : int
                Size of batch to train /finetune the model
            epochs : int
                Number of epochs to train/ finetune the model
            verbose : int
                Verbosity level      
        ...
        self.epochs = epochs
        self.pretrained_weights_path = os.path.join(output_dir, 'tl_pretrained_weights.hdf5')
        self.fine_tuned_weights_path = os.path.join(output_dir, 'tl_fine_tuned_weights.hdf5')
    ...
    def build_TL_model(self, input_shape = (config.PAD_SIZE,1) ):
        """
            Build the transfer learning model with ECG5000 as source dataset
        """
    ...
    def get_metrics_of_task_TL(self, support_set, support_labels, query_set, query_labels):
        """
            Get evaluation metrics of one task of transfer learning model from support set and query set
        ...
        model_checkpoint = tf.keras.callbacks.ModelCheckpoint(self.fine_tuned_weights_path, 
    ...
    def get_metrics_k_shot_TL(self, X, y, k, q, n_tasks):
        """
            Get evaluation metrics of k-shot tasks of the TL model from a dataset
    ...
    def print_metrics_TL(self, X_test, y_test, k_min, k_max, num_tasks, q):
        """
            Print evaluation metrics of k-shot tasks of TL model from a dataset
        ...
        for i in range(k_min, k_max+1):
            ...
            self.TL_accs.append(np.mean(accuracies))
            self.TL_recalls.append(np.mean(recalls))
            self.TL_precisions.append(np.mean(precisions))
            self.TL_F1s.append(np.mean(f1s))

        TL_df = pd.DataFrame()
        TL_df['TL Accuracy'] = self.TL_accs
        TL_df['TL Recall'] = self.TL_recalls
        TL_df['TL Precision'] = self.TL_precisions
        TL_df['TL F1'] = self.TL_F1s