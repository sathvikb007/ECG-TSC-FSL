...
class DTW_Model:
    ...
    def get_metrics_of_task_DTW(self, support_set, support_labels, query_set, query_labels):
        """
            Get evaluation metrics of one task of DTW from support set and query set
        ...
        for query in query_set.values:
            query_distances = []
            # Get last non-zero index of query. Note that DTW does not require padding
            q_index = utils.get_last_nonzero_index(query)

            # Iterate through all support samples and find nearest DTW distance
            for support in support_set.values:
                # Get last non-zero index of support set sample
                s_index = utils.get_last_nonzero_index(support)

                q_vec = query[: q_index + 1]
                s_vec = support[: s_index + 1]
                
                # Get DTW distance between query and support sample
                dist = utils.DTW_distance(q_vec, s_vec)
                query_distances.append(dist)

            # Get the index of the nearest DTW distance and the corresponding label
            pos = np.argmin(query_distances)
    ...
    def get_metrics_k_shot_DTW(self, X, y, k, q, n_tasks):
        """
            Get evaluation metrics of k-shot tasks of DTW from a dataset
        ...
        for i in tqdm.tqdm(range(n_tasks)):
            ...
            acc, recall, precision, f1= self.get_metrics_of_task_DTW(support_set, support_labels, query_set, query_labels)
    ...
    def print_metrics_DTW(self, X_test, y_test, k_min, k_max, num_tasks, q):
        """
        ...
        for i in range(k_min, k_max+1):
            accuracies, recalls, precisions, f1s= self.get_metrics_k_shot_DTW(X_test, y_test, k = i, q = q, n_tasks= num_tasks)
            ...
            self.DTW_accs.append(np.mean(accuracies))
            self.DTW_recalls.append(np.mean(recalls))
            self.DTW_precisions.append(np.mean(precisions))
            self.DTW_F1s.append(np.mean(f1s))
